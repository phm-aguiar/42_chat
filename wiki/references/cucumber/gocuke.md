---
title: "Gocuke — Gherkin-based BDD Testing for Go"
category: references
tags:
  - cucumber
  - bdd
source: "wiki/_raw/regen-networkgocuke 🥒 Gherkin-based BDD testing for go.md"
created: "2026-06-21"
updated: "2026-06-21"
lifecycle: raw
...
- property-based testing via [https://github.com/flyingmutant/rapid](https://github.com/flyingmutant/rapid)
- user-friendly wrapper for data tables
- support for big integers and big decimals (via [https://github.com/cockroachdb/apd](https://github.com/cockroachdb/apd))
...
`gocuke` was inspired by [godog](https://github.com/cucumber/godog) and [gobdd](https://github.com/go-bdd/gobdd). I tried both of these libraries and wanted a specific developer UX that I couldn't achieve with either. godog was not a good fit for the same reasons as that gobdd was created (specifically tight integration with `*testing.T`). Looking at the source code for gobdd, it needed to be updated to a new versions of cucumber/gherkin-go and cucumber/messages-go and significant changes were needed to accommodate this API. So `gocuke` was written. We are happy to coordinate with the authors of either of these libraries at some point to align on common goals.
...
Cucumber [tag expressions](https://cucumber.io/docs/cucumber/api/#tag-expressions) can be used for selecting a subset of tests to run. The command-line option `-gocuke.tags` can be used to specify a subset of tests to run.
...
Property-based tests using [https://github.com/flyingmutant/rapid](https://github.com/flyingmutant/rapid) can be enabled by using `*rapid.T` as the first argument of test methods (after the suite receiver argument). Property-based test cases will be run as many times is rapid is configured to run tests.
...
- We have some steps as defined in \[.pre-commit-config.yaml\] (.pre-commit-config.yaml) that run before commit
	- On mac, simply `brew install pre-commit` to install the library
		...
		- There's a helper script to install the hooks locally [setup-hooks.sh](https://github.com/regen-network/gocuke/blob/main/setup-hooks.sh)
		- pre-commit config is held in [.pre-commit-config.yaml](https://github.com/regen-network/gocuke/blob/main/.pre-commit-config.yaml)
		- More docs: [https://pre-commit.com/](https://pre-commit.com/)
