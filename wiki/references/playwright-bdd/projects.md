...
tags:
  ...
  - projects
created: 2026-06-21
source: "https://vitalets.github.io/playwright-bdd/#/configuration/multiple-projects"
---
...
For convenience, there is a helper function [`defineBddProject()`](#/api?id=definebddproject). In addition to the standard BDD config, it accepts a project name and automatically sets `outputDir` based on that name. The function returns an object `{ name, testDir }`, which can be merged into the project config with the spread operator.
...
A typical use case is an **authentication setup project**. See [Authentication](#/guides/authentication) for a full guide.
