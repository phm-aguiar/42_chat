...
The JUnit reporter now uses Cucumber-compatible test case naming by default. Previously the default was a Playwright-style name (feature › rule › scenario).
...
To disable this validation for all steps in a BDD config, set [`arityCheck: false`](#/configuration/options?id=aritycheck).
