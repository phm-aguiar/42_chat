...
tags:
  ...
  - options
created: 2026-06-21
source: "https://vitalets.github.io/playwright-bdd/#/configuration/options"
---
...
Path(s) to feature files. Can be a directory or [glob pattern](https://github.com/micromatch/picomatch?tab=readme-ov-file#globbing-features). Example: `features/**/*.feature`. If you don't specify a file extension, the default is `*.feature`. Resolved relative to the config file location.

> Since Playwright-BDD **v8**, you can omit the `features` option and define [`featuresRoot`](#/configuration/options?id=featuresroot), which serves as a common base directory for both features and steps. The features glob pattern will be calculated as `{featuresRoot} + /**/*.feature`.
...
Path(s) to step definitions. Can be a directory or [glob pattern](https://github.com/micromatch/picomatch?tab=readme-ov-file#globbing-features). Example: `steps/**/*.ts`. If you don't specify a file extension, the default is `*.{js,mjs,cjs,ts,mts,cts}`. Resolved relative to the config file location.

> Since Playwright-BDD **v8**, you can omit the `steps` option and define [`featuresRoot`](#/configuration/options?id=featuresroot), which serves as a common base directory for both features and steps. The steps glob pattern will be calculated as `{featuresRoot} + /**/*.{js,mjs,cjs,ts,mts,cts}`.
...
- `promptAttachment` *boolean* - Whether to attach AI prompt to failing tests
...
- `fail-on-gen` *(default)* - test files generation will fail and display code [snippets](#/writing-steps/snippets) for missing steps
...
Deprecated, use [`features`](#/configuration/options?id=features) instead.
...
Deprecated, use [`steps`](#/configuration/options?id=steps) instead.
...
Paths to step definitions in [ESM](#/configuration/esm).
