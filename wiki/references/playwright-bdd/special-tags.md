...
To conditionally skip a scenario, use [$test and $tags fixtures](#/writing-steps/bdd-fixtures?id=test-and-testinfo).
...
Both `Scenario 1` and `Scenario 2` will be executed in parallel in separate workers.

When using `@mode:serial`, you can check [Passing data between scenarios](#/writing-steps/passing-data-between-scenarios).
