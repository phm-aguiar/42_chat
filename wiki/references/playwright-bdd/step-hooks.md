...
tags:
  ...
  - afterstep
created: 2026-06-21
source: "https://vitalets.github.io/playwright-bdd/#/writing-steps/hooks/step-hooks"
---
...
The hook function accepts **1 argument** - [test-scoped fixtures](https://playwright.dev/docs/test-fixtures#built-in-fixtures). You can access [$testInfo](https://playwright.dev/docs/api/class-testinfo), [$tags](#/writing-steps/bdd-fixtures?id=tags) and any built-in or custom fixtures. See more details in [BeforeStep API](#/api?id=beforestep).

## AfterStep

Playwright-BDD supports the step-level hook `AfterStep`. It runs **after each invoked step**.

If the step body throws, the thrown value is available as `$step.error` inside `AfterStep`. If a `BeforeStep` hook fails, the step body is not invoked and `AfterStep` does not run for that step. Playwright hard timeouts and interruptions can still abort execution before `AfterStep` runs.

> Do not rely on `$testInfo.status` inside `AfterStep` to detect whether the current step failed. Playwright may still expose `status: "passed"` while the step error is being handled. Use `$step.error` instead, and handle skipped or other control-flow errors according to your project needs.
...
All options are similar to [BeforeStep](#/writing-steps/hooks/step-hooks?id=beforestep).
