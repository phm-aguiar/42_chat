...
tags:
  ...
  - reference
created: 2026-06-21
source: "https://vitalets.github.io/playwright-bdd/#/cli"
---
...
Generates Playwright test files from Gherkin documents (`.feature` files). This command is typically followed by `npx playwright test` to run the generated tests. It also assumes that there is a `playwright.config.(ts|js)` with one or several calls of [`defineBddConfig()`](#/configuration/index).
...
Prints a list of all found step definitions. Useful for [generating BDD scenarios with ChatGPT](#/writing-features/chatgpt).
