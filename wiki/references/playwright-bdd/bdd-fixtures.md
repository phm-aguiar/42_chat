...
> See also [Accessing BDD fixtures in decorator steps](#/writing-steps/decorators?id=accessing-bdd-fixtures).
...
| `docStringType` | `string \| undefined` | Media type of the doc string argument, if present. See [Doc strings](#/writing-steps/doc-strings). |
...
`$step.error` is mainly useful in [AfterStep hooks](#/writing-steps/hooks/step-hooks?id=afterstep). Use it instead of `$testInfo.status` when you need per-step failure handling.
