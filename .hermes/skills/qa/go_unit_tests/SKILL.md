...
metadata:
  hermes:
    ...
    related_skills: [wiki-query, gherkin_scenarios, local_test_runner]
    category: qa
    resources:
      - SKILL.md
...
```
skill_view(name="go_unit_tests", file_path="references/table-driven.md")
skill_view(name="go_unit_tests", file_path="references/edge-cases.md")
```
...
O local_test_runner (skill separada) executara os testes.
