...
(G₀ → G₁ → ... → Gₙ). O LATTE gerencia múltiplos workers (Lead, Dev1, Dev2, QA1, ...) que
...
|---|---|---|---|
| `agent` | Opcional | `Lead`, `Dev1`, `Dev2`, ..., `DevN`, `QA1`, `QA2`, ..., `QAN` | Worker designado para executar esta task no round inicial. Se omitido, o LATTE atribui dinamicamente no primeiro round. |
...
|---|---|---|
| `Lead` | Dev | Coordenador. Gerencia o grafo, reatribui tasks, monitora heartbeats. Sempre presente. |
| `Dev1`, `Dev2`, ..., `DevN` | Dev | Workers de desenvolvimento. Executam tasks com `Papel: Dev`. |
| `QA1`, `QA2`, ..., `QAN` | QA | Workers de QA. Executam tasks com `Papel: QA`. |

O número de workers (`N`) é definido pelo `plan.md` ou pelo operador humano. O `Lead` é fixo;
workers Dev e QA podem ser adicionados/removidos dinamicamente via operadores de grafo.
...
## Exemplo de sumário para clarify() com LATTE

```
tasks.md: 6 tarefas em 3 fases (LATTE enabled, H=4, max-rounds=40)

G₀: 6 nodes, 7 edges, 3 workers (Lead, Dev1, QA1)
  ready: T001 (Lead), T004 (QA1)
  esperando: T002, T003, T005, T006

Fase 1: Fundação (2 tasks)
  T001 Criar modelo Message (Lead, paralelo)
  T002 Criar migration (Dev1, depende T001)

Fase 2: Implementação (3 tasks)
  T003 Criar handler POST /messages (Dev1, depende T001,T002)
  T004 Criar cenários Gherkin (QA1, paralelo)
  T005 Criar testes unitários Message (QA1, depende T001)
