...
```
skill_view(name="sdd-init-repo", file_path="assets/templates/constitution-template.md")
```

Crie `constitution.md` com o conteúdo do template. Crie `tech.md` como placeholder
que referencia `sdd-explore-tech` para preenchimento futuro.

Se preferir automação: `bash scripts/scaffold-sdd.sh` (acesse via
`skill_view(name='sdd-init-repo', file_path='scripts/scaffold-sdd.sh')`).
...
```
skill_view(name="sdd-init-repo", file_path="references/sdd-workflow-agents-md.md")
```
