...
When inserting links in Step 4, apply the link format from `llm_wiki/SKILL.md` (Link Format section) using the `OBSIDIAN_LINK_FORMAT` value. When `OBSIDIAN_LINK_FORMAT=markdown`, compute the relative `.md` path from the **file being edited** to the target page.
...
Only add entries for links added in this cross_linker run — do not touch typed entries that were already present.
