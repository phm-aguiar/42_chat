...
metadata:
  hermes:
    ...
    related_skills: [go-implement, react-implement, local-test-runner]
    category: dev
    created: 2026-06-16
    resources:
      - SKILL.md
...
# build-check — Smoke test e verificação de build
...
- **Não confunda smoke-test com testes unitários**: esta skill só verifica **compilação**. Testes unitários (`go test`) são responsabilidade do agent-qa (`local-test-runner`).
