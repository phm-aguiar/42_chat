...
name: dev_toolkit
description: >
  Toolkit consolidado do agent-dev. 15 modos: go, react, build_check, plan, spike,
  debug, code-review, simplify, node-debug, python-debug, claude-code, codex,
  opencode, hermes-config, skill_forge. Carregue sempre que o agent-dev receber
...
metadata:
  hermes:
    ...
    related_skills: [qa_toolkit, github, sdd]
    category: dev
    resources:
      - SKILL.md
...
# dev_toolkit — 15 Modos do Agent-Dev
...
## `build_check` — Smoke Test (go build + vet + npm build)
...
| Antes de reportar DONE | `build_check` |
